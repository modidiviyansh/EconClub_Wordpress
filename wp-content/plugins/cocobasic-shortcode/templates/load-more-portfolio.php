    ...
    if ($portfolio_load_more_query->have_posts()) :
        while ($portfolio_load_more_query->have_posts()) : $portfolio_load_more_query->the_post();
            ...
            switch ($link_thumb_to):
                ...
                default:
                    $portfolio_content.= '<li id="p-item-' . $post->ID . '" class="grid-item animate element-item ' . $p_size . '"><a class="item-link" href="' . get_permalink() . '" data-id="' . $post->ID . '">';
            endswitch;

            $portfolio_content .= $portfolio_post_thumb . '<div class="portfolio-text-holder"><p class="portfolio-title">' . $p_thumb_title . '</p></div></a></li>';
